...
layer_color_map = [colors.BLUE_NWELL , colors.DARK_GREEN , colors.MAGENTA_METAL2 , colors.CYAN_METAL3 , colors.GREEN_METAL4]
...
class Drawable: 
    ...
    def draw(self): 
        pygame.draw.rect(WIN , self.color , (self.x , self.y , self.width , self.height),border_radius=0)
...
class Tile(Drawable): 
    ...
    def __init__(self , row , col , layer , width ,type = TileType.metal, **kwargs) -> None:
        ...
        self.__padding = kwargs.get("padding" , 4) 
        self.color = None 
    ...
    def update_neighbors(self , grid): 
        ...
        if self.__layer.orientation == LayerOrientation.horizontal: 
            ...
            # EAST 
            if self.__col < ROWS - step and not (grid[self.__layer.index][self.__row][self.__col + step].state == TileState.barrier):
                self.__neighbors.append(grid[self.__layer.index][self.__row][self.__col + step]) 
            # WEST 
            if self.__col > step  and not (grid[self.__layer.index][self.__row][self.__col - step].state == TileState.barrier):
                self.__neighbors.append(grid[self.__layer.index][self.__row][self.__col -step]) 
            ...
            # South  
            if self.__row < ROWS - step and not (grid[self.__layer.index][self.__row + step][self.__col].state == TileState.barrier):
                self.__neighbors.append(grid[self.__layer.index][self.__row + step][self.__col]) 
            # North  
            if self.__row > step and not (grid[self.__layer.index][self.__row - step][self.__col].state == TileState.barrier):
                self.__neighbors.append(grid[self.__layer.index][self.__row - step][self.__col]) 
        ...
        # Up 
        if self.__layer.index < LAYERS and not (grid[self.__layer.index + 1][self.__row][self.__col].state == TileState.barrier):
            self.__neighbors.append(grid[self.__layer.index + 1][self.__row][self.__col]) 
        # Down  
        if self.__layer.index > 0 and not (grid[self.__layer.index - 1][self.__row][self.__col].state == TileState.barrier):
            self.__neighbors.append(grid[self.__layer.index - 1][self.__row][self.__col])
...
def build_cross_grid_layers(count , initial_orientation = LayerOrientation.horizontal) -> list[Layer]:
    layers : list[Layer]= [] 
...
def a_star_router(grid  : list[list[Tile]] , start : Tile, end : Tile , update_function) -> list[Tile] : 
    count = 0 
    open_set = PriorityQueue() 
    came_from = {}

    g_score = {tile : float("inf") for grid_layer in grid for row in grid_layer for tile in row}
    g_score[start] = 0 

    f_score = {tile : float("inf") for grid_layer in grid for row in grid_layer for tile in row}
...
def draw_ui(current_layer): 
    ...
    for i in range(len(layer_color_map)):
        ...
        pygame.draw.rect(WIN , layer_color_map[i] , [x_s + max(w, 50) , y_s + 15 + i * h , small_r_w , small_r_w])
    ...
    small_r_w = SCREEN_WIDTH // ROWS
    pygame.draw.rect(WIN , colors.SILVER_VIA , [x_s + max(w, 50) , y_cursor , small_r_w , small_r_w])

    y_cursor += h

    text = font.render(f"Current Layer : {current_layer+1}" , False , colors.BLACK_CONTACT)
    w , h = text.get_size()
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE: 
                    ...
                    if start is not None and end is not None:  
                        ...
                        for e in end:         
                            p = a_star_router(grid , start , e , lambda : drawer(grid , current_layer))
                            paths += p  
